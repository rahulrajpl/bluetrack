...
class ObluAnalytics():
    ...
    def getThresholdScore(self, data_path='../sensor/GetData/steps.txt'):
        ...
        self.X_train_data = (df[1]+df[2])/2
        # self.X_train = hankel(self.df[:L],self.df[L-1:]) # Creating trajectory matrix
        self.X_train = hankel(self.X_train_data[:L],self.X_train_data[L-1:]) # Creating trajectory matrix
    ...
    def getScore(self, UT, centroid, x, y):
        # print(df.shape)
        df = pd.DataFrame([sum(x)/2 for x in list(zip(list(x),list(y)))])
        self.pca = PCA(n_components = 1)
        # lag_vector =  self.pca.fit_transform(df)
        lag_vector =  np.asarray(df)
